...
页面顶部可切换「时间范围」（近 1 / 7 / 30 天）与以下几种视图标签页：

- **明细**（默认）：列出**每一次调用**的明细——时间、请求的别名（alias）、实际使用的底层模型、Provider、提示 / 补全 Token、状态。非管理员与未勾选「全局视角」的管理员都默认落在此视图，方便直接看到每次调用的别名与真实模型。
- **按账号**（仅管理员勾选「全局视角」后出现）：按**用户名**汇总每个账号的调用数、Token 总量、估算成本与错误率——一眼看清各账号总消耗。
- **按 Key / 按模型 / 按时间**：原有的三维聚合视图，可 CSV 导出（90 天窗口、单文件 10 万行上限、公式注入防护）。

> **查看范围提示**：管理员登录后默认只显示**自己账号**的用量。若想看所有账号（如 Bob 等）的调用明细与按账号汇总，请勾选右上角「**全局视角**」。普通用户（user）只能看到自己归属账号的用量，且左侧菜单仅保留「用量报表」一项。
